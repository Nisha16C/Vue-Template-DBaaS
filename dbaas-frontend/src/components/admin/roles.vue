...
<template>
    ...
    <div v-show="!loading">
        ...
        dark:text-white focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800"><span
                            class="relative text-lg px-5 py-2 duration-75 bg-white dark:bg-gray-900 rounded-md ">
                            Cancel
                        </span></button>
...
export default {
    ...
    methods: {
        ...
        async fetchUserRoles() {
            try {
                const response = await axios.get(
                    `http://127.0.0.1:8000/api/v1/users/${this.userId}/roles/`
                );
        ...
        async handleAssignRoles(selectedRoles) {
            ...
            try {
                const response = await axios.post(`http://127.0.0.1:8000/api/v1/add_roles_to_user/`, {
                    user_id: this.userId,
                    roles: selectedRoles,
                });
